...
- 2025-08-28: Observability — request IDs, metrics, logging
  - Add X-Request-ID middleware and expose id via context
  - Add Prometheus `/metrics` with `http_requests_total` and `http_request_duration_seconds`
  - Switch server logs to slog JSON and add timings for key operations (download URL, opensearch, prerender)
  - Tracks: #48

- 2025-08-28: Frontend — separate layouts (public vs admin)
  - PublicLayout for user-facing pages; AdminLayout for moderation/administration
  - Updated routing to render pages inside appropriate layout wrappers
  - Tracks: #56

- 2025-08-28: Frontend — React Router v7
  - Upgrade to v7 and migrate imports from `react-router-dom` → `react-router` (DOM-specific deep import not needed for our usage)
  - Build verified; no code changes required beyond version bump
  - Tracks: #57

- 2025-08-28: Search — Postgres FTS + API + FE page
  - DB: add GIN index on `to_tsvector('simple', title)` (created on startup)
  - API: `searchGames(q, platform?, yearFrom?, yearTo?, format?, first?, after?) -> GameConnection!` using FTS + filters
  - FE: `/search` page with query + platform filter; paginated results (basic)
  - Tracks: #58


- 2025-08-28: Comments — schema, CRUD, rate limits, UI
  - Ent: add `Comment` with sanitized content; edges to Game/File
  - GraphQL: `addComment`, `editComment`, `deleteComment` (auth; author/admin restrictions)
  - Rate limits: per-minute caps via in-memory limiter keyed by user/IP
  - FE: GameDetail shows comments and add form for authenticated users
  - Tracks: #24, #25

- 2025-08-28: Likes and file reactions
  - Ent: add `GameLike` and `FileReaction` entities with unique (user, game/file)
  - GraphQL: `rateGame` and `reactToFile` mutations; `viewerDidLike` and `reactionSummary` fields
  - FE: GameDetail shows like counts and per-file thumbs up/down with toggle
  - Tracks: #40, closes #31
