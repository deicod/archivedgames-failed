...
func main(){
    ...
    if n == 0 {
        ...
        if err != nil { log.Fatal(err) }
        _, _ = c.File.Create().SetPath("c64/example1.d64").SetOriginalName("Example1.d64").SetNormalizedName("Example 1").SetChecksum("deadbeef").SetSizeBytes(123456).SetSource("s3").SetGame(g1).Save(ctx)

        g2, err := c.Game.Create().SetSlug("example-game-2").SetPlatform(game.PlatformDOS).SetTitle("Example Game 2").Save(ctx)
        if err != nil { log.Fatal(err) }
        _, _ = c.File.Create().SetPath("dos/example2.zip").SetOriginalName("Example2.zip").SetNormalizedName("Example 2").SetChecksum("beadfeed").SetSizeBytes(654321).SetSource("s3").SetGame(g2).Save(ctx)
