...
func (_q *CommentQuery) QueryGame() *GameQuery {
...
func (_q *CommentQuery) QueryFile() *FileQuery {
...
func (_q *CommentQuery) WithGame(opts ...func(*GameQuery)) *CommentQuery {
	query := (&GameClient{config: _q.config}).Query()
...
func (_q *CommentQuery) WithFile(opts ...func(*FileQuery)) *CommentQuery {
	query := (&FileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(comment.FieldSubjectType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Comment.Query().
//		Select(comment.FieldSubjectType).
//		Scan(ctx, &v)
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if _q.withGame != nil || _q.withFile != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withGame; query != nil {
		if err := _q.loadGame(ctx, query, nodes, nil,
			func(n *Comment, e *Game) { n.Edges.Game = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFile; query != nil {
		if err := _q.loadFile(ctx, query, nodes, nil,
			func(n *Comment, e *File) { n.Edges.File = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *CommentQuery) loadGame(ctx context.Context, query *GameQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Game)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Comment)
	for i := range nodes {
		if nodes[i].game_comments == nil {
			continue
		}
		fk := *nodes[i].game_comments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "game_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CommentQuery) loadFile(ctx context.Context, query *FileQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *File)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Comment)
	for i := range nodes {
		if nodes[i].file_comments == nil {
			continue
		}
		fk := *nodes[i].file_comments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "file_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
