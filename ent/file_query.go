...
func (fq *FileQuery) QueryGame() *GameQuery {
	query := (&GameClient{config: fq.config}).Query()
...
func (fq *FileQuery) WithGame(opts ...func(*GameQuery)) *FileQuery {
	query := (&GameClient{config: fq.config}).Query()
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withGame; query != nil {
		if err := fq.loadGame(ctx, query, nodes, nil,
...
func (fq *FileQuery) loadGame(ctx context.Context, query *GameQuery, nodes []*File, init func(*File), assign func(*File, *Game)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "game_files" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
