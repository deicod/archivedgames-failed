...
// The UserShadowFunc type is an adapter to allow the use of ordinary
// function as UserShadow mutator.
type UserShadowFunc func(context.Context, *ent.UserShadowMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserShadowFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserShadowMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserShadowMutation", m)
}
